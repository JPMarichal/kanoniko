"""SQLite implementation of :class:`ProfileStore`.
...
:func:`make_profile_store`, not on this class directly.
"""
...
class SqliteProfileStore:
        ...
        try:
            ...
            try:
                conn.execute(
                    ...
                    "ADD COLUMN disambiguated_counts TEXT NOT NULL DEFAULT '{}'"
                )
    ...
    def get_profile(
        ...
        try:
            if entity_type:
                ...
                row = conn.execute(
                    "SELECT * FROM entity_profiles WHERE entity_name = ? "
    ...
    def count(
        ...
        try:
            ...
            where = " AND ".join(where_clauses)
            row = conn.execute(
                f"SELECT COUNT(*) FROM entity_profiles WHERE {where}", params
            ).fetchone()
            return row[0]
        finally:
            conn.close()
    ...
    def _row_to_profile(row: sqlite3.Row) -> EntityProfile:
        return EntityProfile(
            ...
            disambiguated_counts=(
                json.loads(row["disambiguated_counts"])
                if row["disambiguated_counts"]
                else {}
            ),

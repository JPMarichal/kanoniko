...
| R3 | ✅ En código | `is_garbage()` con mismos buckets que R0 — lo que el cleanup borra es lo que la ingesta rechaza. Ampliado 2026-04-18 con tres reasons nuevos derivados del diagnóstico de ruido: `scripture_ref` (regex multi-idioma EN/ES de libros canónicos + capítulo/versículo), `mojibake` (detecta cp1252-from-utf8: `Ã¡`, `â€™`, `Â­`, clusters de `ÏÎ`), `html_fragment` (tags, `id=/class=/href=` inline, entidades HTML). Validado contra gold standard n=300: 11 % filtrado, 6 % FP rate; ver `docs/kg-noise-diagnostic.md`. |

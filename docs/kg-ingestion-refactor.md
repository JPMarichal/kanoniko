...
| R3 | ✅ En código | `is_garbage()` con mismos buckets que R0 — lo que el cleanup borra es lo que la ingesta rechaza. **Ampliado 2026-04-18/19** con ocho reasons nuevos derivados del diagnóstico de ruido (`docs/kg-noise-diagnostic.md`): `scripture_ref`, `mojibake`, `html_fragment`, `leading_punct`, `sentence_fragment_es`, `lowercase_token`, `markdown_heading`, `measurement`. Plus deterministic family-relation patterns en `family_patterns.py` invocados desde `KGExtractor._emit_family_relations()`. Validado contra gold standard n=300, 0 % FP en cat 1. Aplicado correctivamente al KG vivo: −80,107 nodos (−9 %), +2,053 FATHER_OF (+258 %), +261 SPOUSE_OF (+580 %). Caso testigo: `Amaleki → Abinadom → Chemish` reconstruido desde Omni 1. |

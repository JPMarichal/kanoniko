...
description: NEVER register ManualConfig entries, assign authority, or prepare download scripts without first completing and WRITING the Fase 0 analysis for each material in the backlog
type: feedback
---
...
**Rule:** Before registering any new material in `download_manual.py` or any download script, you MUST first write a "Fase 0 — Anlisis de contenido" entry in `proj/P4-corpus-expansion/03-materials-backlog.md` covering: what it is, who produced it, when, for whom, how referenced, and relationships. The written entry is the verifiable deliverable that proves the gate was passed.
...
3. Write the Fase 0 entry in the backlog — this is the deliverable
4. ONLY THEN proceed to authority assignment (step 3), KG pre-seed (step 4), and config/script (step 5)
